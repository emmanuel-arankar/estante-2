...
import { acceptFriendRequest, rejectFriendRequest, removeFriend, getUsersBatch } from '../services/firestore';
...
const processFriendshipData = (friendships: any[], users: User[], currentUserId: string): FriendshipWithUser[] => {
  return friendships.map(friendship => {
    let otherUserId: string;

    if (friendship.status === 'pending') {
      // Para solicitações pendentes, precisamos identificar quem é o outro usuário
      if (friendship.requestedBy === currentUserId) {
        // Se o usuário atual enviou a solicitação, o outro usuário é o friendId
        otherUserId = friendship.friendId;
      } else {
        // Se o usuário atual recebeu a solicitação, o outro usuário é quem enviou (requestedBy)
        otherUserId = friendship.requestedBy;
      }
    } else {
      // Para amizades aceitas, o outro usuário é sempre o friendId
      otherUserId = friendship.friendId;
    }

    console.log("🔍 friendship", friendship);
    console.log("➡️ otherUserId escolhido:", otherUserId);
      ...
      immer((set, get) => ({
        ...
        subscribeToRealtimeUpdates: (userId: string) => {
          ...
          const processSnapshot = (
            ...
            console.log(`📡 [${type}] raw friendships`, friendships);
          
            const friendIds = friendships
              .map(f => {
                if (type === 'friends' || type === 'sentRequests') {
                  return f.friendId;
                }
                if (type === 'requests') {
                  return f.requestedBy; // Para requests, pegamos quem enviou a solicitação
                }
                return '';
              })
              .filter(Boolean);
