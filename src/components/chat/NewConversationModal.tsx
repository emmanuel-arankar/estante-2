...
export const NewConversationModal = ({ isOpen, onClose }: NewConversationModalProps) => {
  ...
  const searchUsers = async (searchTerm: string) => {
    ...
    setLoading(true);
    try {
      // Buscar por nome de usuário (nickname)
      const nicknameQuery = query(
        collection(db, 'users'),
        where('nickname', '>=', searchTerm.toLowerCase()),
        where('nickname', '<=', searchTerm.toLowerCase() + '\uf8ff'),
        limit(10)
      );
      ...
      // Processar resultados da busca por nickname
      nicknameSnapshot.forEach((doc) => {
        const userData = { id: doc.id, ...doc.data() } as User;
        if (userData.id !== currentUser?.uid && !userIds.has(userData.id)) {
          foundUsers.push(userData);
          userIds.add(userData.id);
        }
      });

      // Processar resultados da busca por displayName
