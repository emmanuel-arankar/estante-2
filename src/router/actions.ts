...
import { signInWithEmailAndPassword, createUserWithEmailAndPassword, updateProfile as updateFirebaseAuthProfile } from 'firebase/auth';
...
export const loginAction = async ({ request }: any) => {
  ...
  try {
    ...
    return redirect('/'); // Redireciona para a Home, que terá o Header atualizado
  } catch (error: any) {
...
export const registerAction = async ({ request }: any) => {
    ...
    try {
        ...
        const newProfileData: Omit<User, 'id'> = {
            displayName: displayName as string, nickname, email: email as string,
            photoURL: user.photoURL || '', bio: '', location: '', website: '',
            birthDate: undefined, joinedAt: new Date(), booksRead: 0,
            currentlyReading: 0, followers: 0, following: 0,
        ...
        queryClient.setQueryData(['users', user.uid], { id: user.uid, ...newProfileData });
...
export const editProfileAction = async ({ request }: any) => {
    ...
    try {
        ...
        if (data.birthDay && data.birthMonth && data.birthYear) {
            birthDate = new Date(parseInt(data.birthYear as string), parseInt(data.birthMonth as string) - 1, parseInt(data.birthDay as string));
        }

        await updateDoc(doc(db, 'users', user.uid), {
            displayName: data.displayName, nickname: data.nickname,
            bio: data.bio || '', location: data.location || '',
            website: data.website || '', birthDate: birthDate,
            updatedAt: new Date(),
        });
